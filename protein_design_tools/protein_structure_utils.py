        ...
        for residue in chain.residues:
            ...
            for atom in residue.atoms:
                if atom_type == "all" or atom.name == atom_type:
                    ...
                    coordinates.append([atom.x, atom.y, atom.z])
                elif atom_type == "sidechain" and atom.name not in [
                    "N",
                    "CA",
                    "C",
                    "O",
...
def get_radgyr_alanine_helix(sequence_length, atom_type="backbone"):
    ...
    if atom_type == "all":
        ...
        radius_of_gyration = (
            0.4704427974768968 * float(sequence_length) + 0.33788445679512336
        )
    elif atom_type == "backbone":
        # values are from a linear regression of a dataset of ideal alanine helices of different lengths
        radius_of_gyration = (
            0.4706800060715209 * float(sequence_length) + 0.17986881616526773
        )
    else:
...
def get_radgyr_ratio(structure, atom_type="backbone", chains=None):
    ...
    structure_radius_of_gyration = get_radgyr(
        structure, atom_type=atom_type, chains=chains
    )
    ...
    alanine_radius_of_gyration = get_radgyr_alanine_helix(
        alanine_helix_length, atom_type=atom_type
    )
