...
class ProteinStructure:
    ...
    def get_sequence_dict(self) -> Dict[str, str]:
        ...
        for chain in self.chains:
            sequence = "".join([res.one_letter_code if res.one_letter_code else 'X' for res in chain.residues])
    ...
    def get_coordinates(
        ...
        atom_type : str, optional
            Type of atoms to retrieve ("all", "backbone", "CA", "non-hydrogen"). Defaults to "all".
        selection : dict, optional
            Dictionary specifying residue selection per chain, e.g., {'A': [1,2,3], 'B': range(10, 20)}.
        ...
        for chain in self.chains:
            if selection and chain.name not in parsed_selection:
                continue
            selected_residues = parsed_selection.get(chain.name, [res.res_seq for res in chain.residues])
