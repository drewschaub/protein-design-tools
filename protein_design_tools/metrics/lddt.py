...
def compute_lddt_jax(P: jnp.ndarray, Q: jnp.ndarray, cutoff=8.0) -> jnp.ndarray:
    ...
    def compute_lddt_for_residue(i, acc):
        ...
        lddt = (overlap.size / neighbors_p.size) * 100.0 if neighbors_p.size > 0 else 100.0
...
def compute_lddt_numpy(P: np.ndarray, Q: np.ndarray, cutoff=8.0) -> float:
    ...
    for i in range(N):
        ...
        lddt = (len(overlap) / len(neighbors_p)) * 100 if len(neighbors_p) > 0 else 100.0
...
def compute_lddt_pytorch(P: torch.Tensor, Q: torch.Tensor, cutoff=8.0) -> torch.Tensor:
    ...
    for i in range(N):
        ...
        neighbors_p = torch.nonzero((distances_p <= cutoff) & (distances_p > 0)).squeeze()
        
        # Compute distances in Q
        distances_q = torch.norm(Q[i] - Q, dim=1)
        neighbors_q = torch.nonzero((distances_q <= cutoff) & (distances_q > 0)).squeeze()
        
        # Compute overlap
        overlap = torch.intersect1d(neighbors_p, neighbors_q)
        lddt = (overlap.numel() / neighbors_p.numel()) * 100 if neighbors_p.numel() > 0 else 100.0
        lddt_scores.append(lddt)
    
    lddt_avg = torch.mean(torch.stack(lddt_scores))
    return lddt_avg

def compute_lddt_tensorflow(P: tf.Tensor, Q: tf.Tensor, cutoff=8.0) -> tf.Tensor:
    """
    Compute a simplified LDDT between two NxD TensorFlow tensors.
    ...
    for i in range(N):
        # Compute distances in P
        distances_p = tf.norm(P[i] - P, axis=1)
        neighbors_p = tf.where((distances_p <= cutoff) & (distances_p > 0))[:, 0]
        
        # Compute distances in Q
        distances_q = tf.norm(Q[i] - Q, axis=1)
        neighbors_q = tf.where((distances_q <= cutoff) & (distances_q > 0))[:, 0]
        
        # Compute overlap
        overlap = tf.sets.intersection(tf.expand_dims(neighbors_p, 0), tf.expand_dims(neighbors_q, 0))
        overlap_size = tf.shape(overlap.values)[0]
        neighbors_p_size = tf.shape(neighbors_p)[0]
        
        # Compute LDDT for residue i
        lddt = tf.cond(
            neighbors_p_size > 0,
            lambda: (tf.cast(overlap_size, P.dtype) / tf.cast(neighbors_p_size, P.dtype)) * 100.0,
            lambda: tf.constant(100.0, dtype=P.dtype)
        )
        lddt_scores.append(lddt)
    
    lddt_avg = tf.reduce_mean(tf.stack(lddt_scores))
    return lddt_avg
